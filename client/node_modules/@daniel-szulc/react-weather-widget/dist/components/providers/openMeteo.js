...
function castData(data, opts) {
  return {
    ...
    temperature: Math.round((0, _unitsConverter.unitConvert)(data.current_weather.temperature + 273.15, opts.tempUnit)),
    humidity: data.hourly.relativehumidity_2m[data.hourly.time.indexOf(data.current_weather.time)],
    weather_desc: codeWeather[data.current_weather.weathercode][1],
    //data.weather[0].description, /////////!!!! from code
    weather_type: codeWeather[data.current_weather.weathercode][0],
    // getWeatherType(data.current_weather.weathercode),
    feels_like: Math.round((0, _unitsConverter.unitConvert)(data.hourly.apparent_temperature[data.hourly.time.indexOf(data.current_weather.time)] + 273.15, opts.tempUnit)),
    wind: Math.round((0, _unitsConverter.unitConvert)(data.current_weather.windspeed, opts.windSpeedUnit) * 10) / 10,
    units: {
...
function localize(opts) {
  ...
  return new Promise(async result => {
    (0, _request.default)(requestOptions).then(data => result(data));
  });
}
  ...
  if (opts) {
    ...
    return new Promise(async result => {
      await localize(opts).then(async res => {
        ...
        await (0, _request.default)(requestOptions).then(response => castData(response, opts)).then(data => result(data));
      });
    });
  }
}

var _default = openMeteo;
